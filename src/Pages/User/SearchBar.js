...
//   useEffect(() => {
//     if (debouncedDeparture || debouncedDestinations.some(Boolean)) {
//       searchFlights({
//         tripType,
...
//       });
//     }
//   }, [debouncedDeparture, debouncedDestinations, tripType, departureDate, arrivalDate, searchFlights]);
...
const SearchBar = ({ searchFlights }) => {
  ...
  // Debounced values
  const debouncedDeparture = useDebounce(departure, 500);  // Added a 500ms delay
  const debouncedDestinations = useDebounce(destinations, 500);  // Added a 500ms delay

  // Memoize search params calculation to avoid unnecessary recalculation
